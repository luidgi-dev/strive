...
The root layout also exports `viewport` with a media-queried `themeColor` (light `#f8f9f9`, dark `#1d2122`, matching the `--background` tokens). The browser/OS status bar — visible on the PWA installed to a home screen — follows the OS color-scheme preference on initial paint. Hex values are required because the `<meta name="theme-color">` tag and `public/site.webmanifest` cannot reference CSS tokens; the manifest also pins `theme_color` and `background_color` to the dark value since Strive is dark-by-default.

To make the status bar follow the **in-app** theme toggle (not just the OS), the root layout also mounts [`DynamicThemeColor`](../components/providers/dynamic-theme-color.tsx) inside `ThemeProvider`. It is a tiny `"use client"` side-effect component that reads `resolvedTheme` from `next-themes` and **replaces** every `meta[name="theme-color"]` node whenever the theme changes — including when the user forces Light/Dark via the Settings toggle while the OS is in the opposite mode. Removing the old node and appending a fresh one is intentional: iOS PWA WebView only re-evaluates the status bar when a `theme-color` meta is *added*. Mutating `content` on an existing node leaves the bar stale until the next navigation gesture (swipe-back, App Switcher), which is exactly the bug we hit before this fix. The media-queried meta tags from `viewport.themeColor` remain useful for the SSR initial paint; `DynamicThemeColor` strips them and takes over once hydrated. The shared hex constants `THEME_COLOR_LIGHT` and `THEME_COLOR_DARK` live in [`lib/theme-colors.ts`](../lib/theme-colors.ts) — a neutral (non-`"use client"`) module so both the server-side `viewport` export and the client-side sync can import them. Inlining them inside `dynamic-theme-color.tsx` would turn the exports into client references and break the server-side import with a `DataCloneError`.
