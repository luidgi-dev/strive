...
export const config = {
  matcher: [
    ...
    // robots.txt, sitemap.xml and opengraph-image are root-level, locale-agnostic
    // metadata routes: the rewrite would prepend /en and 404 them, so crawlers
    // would see the not-found page instead of the real file.
    "/((?!_next/static|_next/image|_vercel|favicon.ico|sw\\.js|robots\\.txt|sitemap\\.xml|opengraph-image|site(?:\\.fr)?\\.webmanifest|.*\\.(?:svg|png|jpg|jpeg|gif|webp)$).*)",
  ],
};