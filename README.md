...
2. Install dependencies:
  ```bash
   npm install
  ```
3. Set up the database. The app reads from Supabase on first load, so a fresh, empty project will error out. Apply the schema, functions, triggers, and seeds with the migration runner — full steps (prerequisites, `DATABASE_URL`) in [`data/README.md`](data/README.md):
  ```bash
